...
class MarkdownTranslateWithImageRequest(BaseModel):
    ...
    llm_model: str = Field("qwen3-vl-flash", description="OpenAI ChatCompletions 模型名称")
    ocr_model: str = Field("qwen-vl-ocr-latest", description="OCR 模型名称")
...
class MarkdownToDocxRequest(BaseModel):
    doc_name: str = Field(..., description="文档名（对应 output/mineru/<name>/ 下目录名，读取 <name>_translate_with_image.md）")
